  ...
  return (
    <section className="py-16 bg-gray-100">
      <div className="max-w-[1500px] mx-auto px-4 relative">
        ...
        <p className="text-gray-500 mb-8">Fetched live from the reviews collection.</p>
                  ...
                  >
                    {slideReviews.map((review) => {
                      ...
                      return (
                        <div
                          key={review.id}
                          className="rounded-2xl bg-white p-5 shadow-sm border border-gray-100 text-gray-800"
                        >
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="text-sm font-medium">{product.brand || "Verified buyer"}</p>
                              <p className="text-xs text-gray-500">{product.category || "From backend data"}</p>
                            </div>
                          </div>
                        </div>
                      );
          ...
          {Array.from({ length: totalSlides }).map((_, index) => (
            <button
              ...
              className={`h-2 rounded-full transition-all duration-300 ${
                currentSlide === index ? "w-10 bg-black" : "w-5 bg-gray-400"
              }`}
            />
          ))}
        </div>

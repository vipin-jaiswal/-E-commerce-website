...
import { AddressListSkeleton, AddressEmptyState } from '../components/address/AddressStates';
...
function AddressesTab() {
  const { addresses, loading, removeAddress, makeDefault } = useAddress();
  ...
  return (
    <div className="bg-white rounded-2xl shadow-card p-6">
      <div className="flex items-center justify-between mb-6">
        <h2 className="font-display text-xl font-semibold text-charcoal">My Addresses</h2>
        {addresses.length > 0 && (
          <button
            type="button"
            onClick={openAdd}
    ...
    return (
      <div className="max-w-site mx-auto px-6 py-10">
        <div className="rounded-2xl border border-border bg-white p-8 text-center shadow-card">
          <h1 className="font-display text-3xl font-semibold text-charcoal mb-3">My Account</h1>
          <p className="text-muted mb-6">Please sign in to view your account details, orders, and wishlist.</p>
          <Link to="/login" className="inline-flex rounded-full bg-charcoal px-6 py-3 text-sm font-semibold text-white transition-colors hover:bg-accent">
            Sign In
          </Link>
        </div>
      </div>
